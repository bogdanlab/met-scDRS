...
variance.directory <- '/u/project/geschwind/lixinzhe/scDRS-output/magma-out/Kangcheng-gs/null-simulation-95percentile/OCT22-2024-ivd/TOP_VARIANCE/'
random.directory <- '/u/project/geschwind/lixinzhe/scDRS-output/magma-out/Kangcheng-gs/null-simulation/OCT22-2024-ivd/RANDOM/'
abundance.directory <- "/u/project/geschwind/lixinzhe/scDRS-output/magma-out/Kangcheng-gs/null-simulation-95percentile/OCT22-2024-ivd/TOP_FRACTION/"
...
for (simulation.mode in names(scDRS.directories)) {
    ...
    for (gene.number in names(ranked.p.distribution)) {
        for (rank.cell in names(ranked.p.distribution[[gene.number]])) {
            ...
            plot.df <- data.frame(
                ...
                theoratical = -log10(as.numeric(gsub('rank', '', rank.cell)) / length(ranked.p.distribution[[gene.number]]))
                );
    ...
    output.path <- paste0(
        ...
        '-',
        simulation.mode,
        '-95quantile-pvalue-pvalue-plot.png'
        );
    gplot <- ggplot(
        plot.df,
    ...
    output.path <- paste0(
        ...
        '-',
        simulation.mode,
        '-95quantile-pvalue-pvalue-plot-with-legend.png'
        );
    gplot <- ggplot(
        plot.df,
        aes(x = theoratical, y = mean, group = feature_number, color = feature_number)) +
...
# s.d: high variance: 0.00567389; random: 0.004768789; abundance: 0.0181488