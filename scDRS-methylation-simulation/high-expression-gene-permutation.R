### random-genes-permutation.R ####################################################################
# purpose: permute some genes to make random null gene set for null simulations
...
### PERMUTATION ###################################################################################
# compute the top expression:
feature.abundance <- colSums(fraction);
top.abundance.gene <- feature.abundance[feature.abundance > quantile(feature.abundance, probs = quantile.cutoff)];

for (simulation in seed) {
    set.seed(simulation + gene.number);

    # grab out the permuted genes:
    random.genes <- sample(names(top.abundance.gene), size = gene.number, replace = FALSE);
    ...
    # check that the genes that were selected actually have the highest expression:
    sorted.abundance <- sort(feature.abundance, decreasing = TRUE);
    gene.only <- names(decomposed.random.gs);
    stopifnot(gene.only %in% names(sorted.abundance[1:ceiling(ncol(fraction) * (1 - quantile.cutoff))]));
    ...
    ### OUTPUT ####################################################################################
    output.path <- paste0(output.dir, 'seed-', simulation, '-high-fraction-genes-permuted-weights-', gene.number, '-genes.gs');
    gs.df <- data.frame(
        TRAIT = paste0('NULL_SIM_TOP_FRACTION_', gene.number, '_genes_seed_', simulation),
        GENESET = random.gs
        );
