...
require(docopt)
'Usage:
    random-genes-permutation.R [--data_matrix <input> --gs_file <gene_sets> --gene_number <number> --replication <num_rep> --output_dir <output>]
...
for (simulation in seed) {
    ...
    random.genes <- sample(colnames(fraction), size = gene.number, replace = FALSE);
    ...
    ### OUTPUT ####################################################################################
    output.path <- paste0(output.dir, 'seed-', simulation, '-random-genes-permuted-weights-', gene.number, '-genes.gs');
    gs.df <- data.frame(
        TRAIT = paste0('NULL_SIM_RANDOM_', gene.number, '_genes_seed_', simulation),
        GENESET = random.gs
        );
