...
pickled_file = "/u/home/l/lixinzhe/project-geschwind/data/GSE215353/processed/production/met_scdrs_processed-mch-v1_1_1_rc1.pkl"

with open(pickled_file, "rb") as f:
    adata = pickle.load(f)

gene_statistics = adata.uns['SCDRS_PARAM']['GENE_STATS']

dict_gs = met_scdrs.util.load_gs(
    '/u/project/geschwind/lixinzhe/scDRS-output/magma-out/Kangcheng-gs/gs_file/magma_10kb_top1000_zscore.75_traits.rv1.gs',
    src_species='human',
    dst_species='human',
...
# from the gene statistics, grab out the bins and decompress it:
    ...
    for disease, geneset in dict_gs.items():
        ...
        if axis_num == 3:
            ...
            for plot_i, (gene_length_bin, wide_matrix) in enumerate(disease_bin_decompressed.items()):
                ...
                # make the heatmap
                cmap = LinearSegmentedColormap.from_list("white_to_red", ["#FFFFFF", "#de2d26"])
                cmap.set_bad(color='lightgray')
                sns.heatmap(fg_bg_ratio, ax = axes[plot_i], annot=True, fmt='.0f', annot_kws = {'size': 8}, cmap=cmap, cbar=True, vmin= 0, vmax = 100, linewidths = 0.5, linecolor = 'lightgray', cbar_kws={'label': '% fg/bg'})
...
ctrl_match_bin(
    preprocessed_h5ad = adata,
    dict_gs = dict_gs,
    ctrl_match_key = 'mean_var_length',
    plot_dir = '/u/home/l/lixinzhe/project-geschwind/plot/met_scdrs_revision/updated_bin_plot/')