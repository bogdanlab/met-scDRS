...
mch.directory <- "/u/project/geschwind/lixinzhe/scDRS-output/fraction-gse132489-methyl//mch/KC/all/";
mcg.directory <- "/u/project/geschwind/lixinzhe/scDRS-output/fraction-gse132489-methyl//mcg/KC/all/";
p.cutoff <- 0.1;
...
for (modality in c('mch', 'mcg')){
    ...
    source('/u/home/l/lixinzhe/project-github/scDRS-applications/spell-book/get-significant-matrix.R')
    ...
    output.path <- paste0(
        ...
        '-',
        annotation,
        '-gse132489-fraction-mch-74traits-cell-class-significance-proportion',
        '.png'
        );
    png(
    ...
    heatmap.legend.param <- list(at = c(0, 0.05));
    ...
    sig.matrix.proportion.collection[[modality]] <- sig.matrix.proportion;
}

### VISUALIZATION FOR PT-L5, ANP, GFRA1, DG-po #################################################### 
annotation <- 'SubType'
significance.matrix <- sig.matrix.proportion.collection[['mch']][[annotation]];
feature.keep <- c(
...
output.path <- paste0(
    ...
    '-mch-',
    annotation,
    '-ANP-PTL5-Gfra1-DGpo-gse132489-fraction-mch-74traits-cell-class-significance-proportion',
    '.png'
    );
png(
    filename = output.path,
