...
######                                    Processing                                 ######
###########################################################################################
QCed_dir = '/u/project/geschwind/lixinzhe/data/GSE215353/feature_agg_with_coverage/merged_QCed/'
coverage_dir = '/u/project/geschwind/lixinzhe/data/GSE215353/feature_agg_with_coverage/merged/'

# this is the prefix for the files
merge_date = '07302026'
qc_date = '073126'

methylation_modality = ['CHN', 'CGN']
aggregation_features = ['exon', 'intron', 'promoter']

for agg_feature in aggregation_features:
    for methyl_mode in methylation_modality:
        coverage_path = f'{coverage_dir}merged_{merge_date}_{methyl_mode}_{agg_feature}_coverage_raw.h5ad'
        qced_path = f"{QCed_dir}merged_{qc_date}_{methyl_mode}_{agg_feature}_QC.h5ad"
        
        # make sure they exist:
        assert os.path.exists(coverage_path)
        assert os.path.exists(qced_path)
        
        # load them in:
        coverage = ad.read_h5ad(coverage_path)
        qced_fraction = ad.read_h5ad(qced_path)
        
        # output the out_df:
        cov_df = generate_control_metric(
            qced_fraction,
            coverage,
            covariate_file_output = f'{QCed_dir}/{methyl_mode}_{agg_feature}_full.cov'
            )
        
        # remove the met_count if it was CpG methylation, because of VIF:
        if methyl_mode == 'CGN':
            cov_df = cov_df.drop(columns = ['met_count'])
            cov_df.to_csv(f'{QCed_dir}/{methyl_mode}_{agg_feature}_full.cov', index = False, sep = '\t')
