...
###########################################################################################
# fixed overlap
output_dir="/u/home/l/lixinzhe/project-geschwind/port/scratch/revision/power_simulation/fixed-overlap/informed_high/"
mch_h5ad_dir="/u/scratch/l/lixinzhe/revision_scratch/simulation/fixed-overlap/informed_high/"
mkdir -p $output_dir

# from the data, grab out the h5ad files and submit job for fixed overlap:
effect_sizes=("0.81" "0.82" "0.83" "0.84" "0.85")
for size in "${effect_sizes[@]}"
do
    for mch_h5ad in ${mch_h5ad_dir}seed-*-effect-${size}-overlap-0.5-causal-simulation.h5ad; do
        # for each of the gs file submit a job:
...
# from the data, grab out the h5ad files and submit job for fixed overlap:
effect_sizes=("0.86" "0.87" "0.88" "0.89")
for size in "${effect_sizes[@]}"
do
    for mch_h5ad in ${mch_h5ad_dir}seed-*-effect-${size}-overlap-0.5-causal-simulation.h5ad; do
        # for each of the gs file submit a job:
...
# move all of the results into the same file directory:
mv /u/home/l/lixinzhe/project-geschwind/port/scratch/revision/power_simulation/fixed-overlap/informed/* /u/home/l/lixinzhe/project-geschwind/port/scratch/revision/power_simulation/fixed-overlap/informed_high/ 

###########################################################################################
######                                    Fixed effect                               ######
###########################################################################################
# fixed effect:
output_dir="/u/home/l/lixinzhe/project-geschwind/port/scratch/revision/power_simulation/fixed-perturbation/informed/"
mch_h5ad_dir="/u/scratch/l/lixinzhe/revision_scratch/simulation/fixed-perturbation/informed/"
mkdir -p $output_dir

# from the data, grab out the h5ad files and submit job for fixed overlap:
for mch_h5ad in ${mch_h5ad_dir}seed-*-effect-0.7-overlap-*-causal-simulation.h5ad; do
    # for each of the gs file submit a job:
    echo "read mch file:"
    echo "$mch_h5ad"
    basic_name=$(basename "$mch_h5ad")
