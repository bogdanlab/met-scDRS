...
###########################################################################################
file_groups = file_info.groupby(["file_mc_type", "file_type"])
...
for (mc_type, data_type), sub in file_groups:
    
    # initiate empty adata:
    merged_adata = None
    
    # print a message:
    print(f'Merging within {mc_type} and {data_type}')
    print('')
    ...
    # output:
    merged_adata.write_h5ad(f"{output_dir}/merged/merged_{today}_{mc_type}_gene_body_{data_type}_raw.h5ad")
