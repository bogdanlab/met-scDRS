...
for (trait in c('PASS_MDD_Howard2019.score.gz')){
    # trait file:
    trait.file = paste0('/u/home/l/lixinzhe/project-cluo/result/met-scDRS/revision/v1.1/ges215353_full/mean_var_length_arcsine/', trait)
    # read in the results:
...
for (trait in c('PASS_MDD_Howard2019.score.gz', 'PASS_BIP_Mullins2021.score.gz', 'PASS_Schizophrenia_Pardinas2018.score.gz')){
    # trait file:
    disease_name = gsub('.score', '', trait)
    trait.file = paste0('/u/home/l/lixinzhe/project-cluo/result/met-scDRS/revision/v1.1/ges215353_full/mean_var_length_arcsine/', trait)
    # read in the results:
    trait.score <- read.table(
        file = trait.file,
    ...
    for (cell_type in unique(meta$X_MajorType)){
        ...
        if (length(unique(cell_type_meta$subtype)) > 1){  
            ...
            gplot <- ggplot(plot.df, aes(x = UMAP_1, y = UMAP_2)) +
                geom_point(data = plot.df[insignificant.cell, ], colour = 'grey') +
                geom_point(data = plot.df[significant.cell, ], aes(colour = met_scdrs)) +
                #limit found by min and max over three scores
                scale_color_gradient(low = "#fee0d2", high = "#de2d26", limits = c(2.6, 6)) +
                theme_classic() +
                ggtitle(paste0(cell_type, ' ', disease_name)) +
                theme(plot.title = element_text(hjust=0.5)) +
                xlab('UMAP 1') +
                ylab('UMAP 2') +
                labs(color = "met-scDRS") +
            ...
            # draw out the plot:
            output.dir <- '/u/home/l/lixinzhe/project-geschwind/plot/subtype-analysis/'
            output.path <- paste0(output.dir, system.date, '-', disease_name, 'scDRS-score-umap-', gsub('/','-', cell_type), '-only.png')
            png(
                filename = output.path,
                width = 14,
                height = 14,
...
#     system.date <- Sys.Date()
#     p.cutoff <- 0.1
#     print(max(trait.score$zscore))
#     print(min(trait.score$zscore[trait.score$fdr <= 0.1]))
# }
